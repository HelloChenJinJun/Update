package com.anew.update.interceptor;
...
public class IndexInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (!url.contains(ConstantUtil.BASE_VIDEO_URL)) {
            ...
            String iii = sharedPreferences.getString(ConstantUtil.III_SESSION, null);
            StringBuilder cookie = new StringBuilder("_ga=GA1.2.1176342053.1543471162;_gid=GA1.2.1372733798.1543471162;_gat=1;");
            if (iii != null) {
                cookie.append(iii);
            }
            String php = sharedPreferences.getString(ConstantUtil.PHPSESSIID, null);
            if (php != null) {
                cookie.append(";").append(php);
            }
            String gsp = sharedPreferences.getString(ConstantUtil.GSP, null);
            if (gsp != null) {
            ...
            if (cookieList != null && cookieList.size() > 0) {
                ...
                for (String s :
                    ...
                    if (s.contains("iii_Session")) {
                        share.putString(ConstantUtil.III_SESSION, s.substring(0, s.indexOf(";")))
                                .apply();
                    } else if (s.contains("PHPSESSIID")) {
                        share.putString(ConstantUtil.PHPSESSIID, s.substring(0, s.indexOf(";")));
                    } else if (s.contains("_gsp")) {
                        share.putString(ConstantUtil.GSP, s.substring(0, s.indexOf(";")));
                    }
                }
                share.apply();
            }
